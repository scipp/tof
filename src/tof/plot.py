...
def plot(
    ...
    if ax is None:
        ...
        fig = ax.get_figure()
    furthest_detector = max(result._detectors.values(), key=lambda d: d.distance)
    ...
    if blocked_rays > 0:
        ...
        if nrays > blocked_rays:
            ...
            inds = slice(None)
        birth_times = result._pulse.birth_times[inv_mask][inds]

        components = sorted(
            chain(result._choppers.values(), [furthest_detector]),
    ...
    if max_rays > 0:
        ...
        if (max_rays is not None) and (len(tofs) > max_rays):
            ...
            inds = slice(None)
        birth_times = result._pulse.birth_times[furthest_detector._mask][inds]
        wavelengths = result._pulse.wavelengths[furthest_detector._mask][inds]
        ...
        result._add_rays(
            ...
            wav_min=result._pulse.wav_min,
            wav_max=result._pulse.wav_max,
        )

    tof_max = tofs.max().value
    # Plot choppers
    for ch in result._choppers.values():
        ...
        y[2::3] = None
        ax.plot(x, y, color="k")
        ax.text(tof_max, ch.distance.value, ch.name, ha="right", va="bottom", color="k")

    # Plot detectors
    for det in result._detectors.values():
        ax.plot([0, tof_max], [det.distance.value] * 2, color="gray", lw=3)
        ax.text(0, det.distance.value, det.name, ha="left", va="bottom", color="gray")

    # Plot pulse
    t_start = result._pulse.t_start.to(unit='us').value
    ax.plot(
        [t_start, result._pulse.t_end.to(unit='us').value],
        [0, 0],
        color="gray",
        lw=3,
    )
